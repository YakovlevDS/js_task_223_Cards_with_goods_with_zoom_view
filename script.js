$(function(){
  ...
  pintarCards($('.cards .contenedorCards'), dataCards);
  ...
  function detallesRapidos(datos){
    const { codigoProducto, descripcionLarga, existencia, image, nombre, precio, precioOferta } = datos;
          ...
          <div class="modalContainer">
            <div class="topContent">
              <div class="imageContainer zoom_section">
                <div class="zoom_launcher zoomWatch" title="Ampliar image">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
  ...
  function zoomImg(zoomArea, escala = 3){
    if ($(window).width() > 768) {
      ...
      const urlImage = zoomArea.find('.zoom_imgSource')[0].style.backgroundImage;
      const dimensiones = {
        image: {width: image.outerWidth(), height: image.outerHeight()},
        lupa: {width: image.outerWidth() / escala, height: image.outerHeight() / escala}
      };
      const estiloInicial = {
        backgroundImage: urlImage,
        backgroundSize: parseInt(image.css('padding')) ? `calc(100% - (${(parseInt(image.css('padding')) / 2) * escala}px))` : 'contain',
        transform: `scale(${escala}) translateX(${dimensiones.image.width / escala}px) translateY(${dimensiones.image.height / escala}px)`
      };
      image.append(`<div class="zoom_lupa"></div>`);
      ...
      const lupa = image.children('.zoom_lupa');
      scaleArea.fadeIn();
      ...
      image.mousemove(function(e){
        ...
        let centroImage = {
          x: (image.offset().left + (dimensiones.image.width / 2)),
          y: (image.offset().top + (dimensiones.image.height / 2))
        }
        let posicionLupa = {
          x: centroImage.x - movimientoLupa.x,
          y: centroImage.y - movimientoLupa.y
        }
        let transformLupa = {
          x: (movimientoLupa.x - (dimensiones.lupa.width / 2)) - image.offset().left,
          y: (movimientoLupa.y - (dimensiones.lupa.height / 2)) - image.offset().top
        }
